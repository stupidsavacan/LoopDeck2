...
LoopDeck Flow は、保存済みの教材・回答履歴・復習状態から「今やる数分」を自動構成し、考える前に学習へ入れるローカルファーストの学習ランナーです。

この `reinterpretation/chat-redesign` ブランチは、既存LoopDeck2のPack・採点・履歴・SRS・インポート互換をデータエンジンとして継承し、その上の情報設計とPlayerを別製品として組み直した再解釈版です。`main` へは自動マージしません。

## Flowの使い方

- 起動地点は `#today`。復習予定、苦手、未回答を1つの短いFlowにまとめます。
- 基本操作は「今やる5分」の1回だけ。10分・20分にも変更できます。
- 5問ごとにCheckpointを挟み、途中で閉じてもIndexedDBから再開できます。
- 「集中期間」を設定すると、期限・1日の時間・対象教材だけをローカル設定として重ねられます。Pack形式は変わりません。
- SRS・弱点・新規は表示上だけ統合し、内部の理由は `due` / `weak` / `new` として保持します。
- 「10秒間未入力なら答えを表示」は「その他 → 学習の動き」からFlow全体に設定できます。入力、選択、貼り付け、ヒント、IME、タブ非表示を考慮します。
...
- 利用可能な問題数: 1,112問
- SRS期限、履歴ベースの弱点、未回答を合成するStudyPlanEngine
- 行動へつながるToday / Library / Progress / Moreの4階層
- 明示的な状態遷移、保存完了後の自動進行、5問Checkpointを持つ新Player
- IndexedDB v4の中断可能なFlowセッション
...
Flowは両方をUI上で1つの計画に合成しますが、理由と順位は失いません。1つ目のスケジューラーは、SM-2 風のシンプルな実装です。
